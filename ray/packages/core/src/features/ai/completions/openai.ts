      ...
      try {
        if ('attachments' in message.content) {
          for (const attachment of message.content.attachments) {
            ...
            const file = readdirSync(dir).find(file => file.includes(attachment.id)) || ''
            debug.info('Attachment file:', resolve(dir, file))
            const image = readFileSync(resolve(dir, file)).toString('base64')
            images.push({
              type: 'image_url',
              image_url: {
